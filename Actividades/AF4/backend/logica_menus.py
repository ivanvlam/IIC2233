    ...
    def golpear_jugador(self):
        if not self.jugador_defendiendo:
            ...
            if self.vida_jugador <= 0:
                ...
                self.senal_enviar_actualizacion.emit({'comando': 'perder'})
            else:
                self.senal_enviar_actualizacion.emit({'comando': 'dano_jugador', \
                    'valor': self.vida_jugador})
        
        # Implementado para que enemigo pueda atacar mientras estoy defendiendo
        else:
            self.senal_enviar_actualizacion.emit({'comando': 'dano_jugador', \
                'valor': self.vida_jugador})
