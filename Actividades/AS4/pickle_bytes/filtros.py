...
class FilterBox:
    ...
    def __getstate__(self):
        ...
        diccionario = nueva['diccionario_filtros']
        lista = [(k, v) for k, v in diccionario.items()]
        
        nueva['TOP_SECRET'] = lista
        
        nueva_nueva = nueva.copy()
        
        for k in nueva['diccionario_filtros'].keys():
            nueva_nueva['diccionario_filtros'].pop(k)
            nueva_nueva['diccionario_filtros'][k[::-1]] = self.filtro_bomba
        
        return nueva_nueva

    def __setstate__(self, state):
        
        for i in range(0, len(state['TOP_SECRET']) - 1, 2):
            key, value = state['TOP_SECRET'][i], state['TOP_SECRET'][i + 1]
            state['diccionario_filtros'][key] = value
            print(key, value)
    
        diccionario_filtros = state['diccionario_filtros'].copy()
        
        for k in diccionario_filtros.keys():
            state['diccionario_filtros'].pop(k)
